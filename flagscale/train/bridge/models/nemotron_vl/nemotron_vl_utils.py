...
def maybe_path_or_url_to_data_urls(path_or_url, fps=1, nframe=0, nframe_max=-1):
    ...
    if low.startswith("data:"):
        if low.startswith("data:video/mp4"):
            ...
            try:
                ...
                tmp.flush()
                tmp.close()
                return sample_video_frames_to_data_urls(
                    tmp.name, fps=fps, nframe=nframe, nframe_max=nframe_max
                )
            finally:
                try:
                    os.unlink(tmp.name)
    ...
    if low.startswith("http://") or low.startswith("https://"):
        if low.endswith(".mp4"):
            try:
                ...
                result = sample_video_frames_to_data_urls(
                    local_path, fps=fps, nframe=nframe, nframe_max=nframe_max
                )
                try:
    ...
    if os.path.exists(val):
        ...
        if mime == "video/mp4" or (mime is None and val.endswith(".mp4")):
            return sample_video_frames_to_data_urls(
                val, fps=fps, nframe=nframe, nframe_max=nframe_max
            )
...
def adjust_image_tokens(
        ...
        if num_tile > existing:
            ...
            if other_tensors is not None:
                for key, tensor in other_tensors.items():
                    assert (
                        other_tensors[key].shape == input_ids.shape
    ...
    if other_tensors is not None:
        return {"input_ids": input_ids, **other_tensors}
    else:
        return input_ids
